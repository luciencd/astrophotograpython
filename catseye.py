...
#LIGHTS
red_lights = map(lambda t:fits.open(source+'catseye-'+str(str(t).zfill(3))+'Red.fit'),range(1,4))
green_lights = map(lambda t:fits.open(source+'catseye-'+str(str(t).zfill(3))+'Green.fit'),range(1,4))
blue_lights = map(lambda t:fits.open(source+'catseye-'+str(str(t).zfill(3))+'Blue.fit'),range(1,4))
...
offsets = offsetter(3,3,-9.0,0.0,459,315,+5.0,0.0)
print offsets
...
rgbfile = RGBFile()
rgbfile.overwrite("results/","catseye",stacks[0],stacks[1],stacks[2])
